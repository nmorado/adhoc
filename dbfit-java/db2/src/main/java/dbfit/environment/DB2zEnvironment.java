...
@DatabaseEnvironment(name="DB2z", driver="com.ibm.db2.jcc.DB2Driver")
public class DB2zEnvironment extends AbstractDbEnvironment {

    public DB2zEnvironment(String driverClassName) {
    ...
    public Map<String, DbParameterAccessor> getAllColumns(String tableOrViewName)
        ...
        String qry = " select colname as column_name, typename as data_type, length, "
                + "	'P' as direction from SYSIBM.SYSCOLUMNS where ";
        if (qualifiers.length == 2) {
            qry += " lower(tbcreator)=? and lower(tbname)=? ";
        } else {
            qry += " (lower(tbname)=?)";
        }
        qry += " order by name";
    ...
    public Map<String, DbParameterAccessor> getAllProcedureParameters(
        ...
        String qry = " select parmname as column_name, typename as data_type, length, "
                + "	rowtype as direction, ordinal from SYSIBM.SYSPARMS  where ";
        if (qualifiers.length == 2) {
            qry += " lower(schema)=? and lower(name)=? ";
